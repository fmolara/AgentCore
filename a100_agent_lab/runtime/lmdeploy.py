...
class LMDeployRuntime(Runtime):
    ...
    def _wait_ready(self, timeout: float) -> None:
        ...
        while time.perf_counter() < deadline:
            if self.process is not None and self.process.poll() is not None:
                raise RuntimeError(f"LMDeploy server exited with code {self.process.returncode}")
            try:
                self._get("/v1/models", timeout=2)
                return
                ...
                last_error = exc
                time.sleep(1)
        raise TimeoutError(f"LMDeploy server did not become ready within {timeout}s: {last_error}")
    ...
    def _stream_chat(self, payload: dict[str, Any]) -> Iterator[str]:
        ...
        try:
            ...
            raise RuntimeError(f"LMDeploy HTTP {exc.code}: {body[:1000]}") from exc
    ...
    def _server_log_path(self) -> Path:
        ...
        return log_dir / f"lmdeploy-server-{stamp}.log"
