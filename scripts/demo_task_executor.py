...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Demonstrate AgentCore task executor.")
    parser.add_argument("--config", default="config/sglang-a100.yaml", help="AgentLab YAML configuration path.")
    parser.add_argument(
        "--workspace",
        default=str(ROOT / "workspace" / "task-executor-demo"),
...
def main() -> None:
    ...
    agent = lab.create_agent(
        ...
        workspace_metadata={"purpose": "task executor demo"},
    )
    ...
    if agent.git.status().stdout.strip():
        ...
        agent.git.commit("Prepare task executor demo")

    task = agent.create_task(
        title="Executor parser edit",
        description="Run a fixed sequence of file, checkpoint, Git, and report actions.",
    )
    executor = TaskExecutor(agent)
    result = executor.execute(
        task,
        [
            CreateCheckpointAction("before edit", "Clean parser baseline."),
            ReadFileAction("parser.c"),
            ReplaceTextAction("parser.c", "return 0;", "return 1;"),
            CreateCheckpointAction("after edit", "Parser returns one token."),
            GitDiffAction(),
            TaskReportAction(),
        ],
    )
