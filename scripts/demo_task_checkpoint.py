...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Demonstrate AgentCore task checkpoints.")
    parser.add_argument("--config", default="config/sglang-a100.yaml", help="AgentLab YAML configuration path.")
    parser.add_argument(
        "--workspace",
        default=str(ROOT / "workspace" / "task-checkpoint-demo"),
...
def main() -> None:
    ...
    agent = lab.create_agent(
        ...
        workspace_metadata={"purpose": "task checkpoint demo"},
    )
    ...
    if agent.git.status().stdout.strip():
        ...
        agent.git.commit("Prepare parser checkpoint demo")

    task = agent.create_task(
        title="Refactor parser",
        description="Record explicit checkpoints while editing parser.c.",
    )
    task.start()

    agent.files.replace_text("parser.c", "return 0;", "return 1;")
    first = task.create_checkpoint("first edit", "Parser returns one token.")

    agent.files.replace_text("parser.c", "return 1;", "return 2;")
    second = task.create_checkpoint("second edit", "Parser returns two tokens.")

    print(json.dumps([first.as_dict(), second.as_dict()], indent=2, sort_keys=True))
