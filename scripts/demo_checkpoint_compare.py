...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Demonstrate AgentCore checkpoint comparison.")
    parser.add_argument("--config", default="config/sglang-a100.yaml", help="AgentLab YAML configuration path.")
    parser.add_argument(
        "--workspace",
        default=str(ROOT / "workspace" / "checkpoint-compare-demo"),
...
def main() -> None:
    ...
    agent = lab.create_agent(
        ...
        workspace_metadata={"purpose": "checkpoint compare demo"},
    )
    ...
    if agent.git.status().stdout.strip():
        ...
        agent.git.commit("Prepare checkpoint compare demo")

    task = agent.create_task(
        title="Compare parser edits",
        description="Record and compare two parser checkpoints.",
    )
    task.start()
