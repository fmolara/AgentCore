...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Demonstrate AgentCore checkpoint restore planning.")
    parser.add_argument("--config", default="config/sglang-a100.yaml", help="AgentLab YAML configuration path.")
    parser.add_argument(
        "--workspace",
        default=str(ROOT / "workspace" / "checkpoint-restore-plan-demo"),
...
def main() -> None:
    ...
    agent = lab.create_agent(
        ...
        workspace_metadata={"purpose": "checkpoint restore plan demo"},
    )
    ...
    if agent.git.status().stdout.strip():
        ...
        agent.git.commit("Prepare checkpoint restore plan demo")

    task = agent.create_task(
        title="Plan parser restore",
        description="Plan restoring a parser checkpoint without changing files.",
    )
    task.start()

    agent.files.replace_text("parser.c", "return 0;", "return 1;")
    checkpoint = task.create_checkpoint("first edit", "Parser returns one token.")
    ...
    if before != after:
        raise RuntimeError("restore planning modified the workspace")

    print(json.dumps(plan.as_dict(), indent=2, sort_keys=True))
    print("workspace unchanged: true")
